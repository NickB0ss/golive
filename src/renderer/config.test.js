...
test('load ignora knownHosts de config antiga sem expor esse campo removido', () => {
  // Configs gravados pela versao B3 ainda podem existir no disco. O campo
  // removido nao pode impedir a abertura nem voltar a existir na memoria.
  const cfg = load(JSON.stringify({ name: 'Ana', knownHosts: [{ address: '100.64.0.8:9000', lastSeenAt: 1 }] }));
  assert.equal(cfg.name, 'Ana');
  assert.equal(Object.hasOwn(cfg, 'knownHosts'), false);
});
